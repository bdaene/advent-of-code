...
def solve(program):
    ...
    def get_beam(x, y):
        ...
        execute(program, input_func=input_func().__next__, output_func=output_func)

        return beam

    pos_x, pos_y = 0, 0
    beam_bottom_left, beam_top_right = False, False
    while not (beam_bottom_left and beam_top_right):
        if not beam_bottom_left:
            pos_x += 1
        else:
            pos_y += 1
        beam_bottom_left = get_beam(pos_x, pos_y + 99)
        beam_top_right = get_beam(pos_x + 99, pos_y)
