    ...
    for p2 in path1[1:]:
        ...
        for p4 in path2[1:]:
            ...
            if intersection is not None and intersection != (0, 0):
                steps = steps1 + steps2 + abs(intersection[0]-p1[0]) + abs(intersection[0]-p3[0]) + abs(intersection[1]-p1[1]) + abs(intersection[1]-p3[1])
                print(intersection, steps)
                if best is None or steps < best[0]:
                    best = steps, intersection
            steps2 += abs(p3[0]-p4[0]) + abs(p3[1]-p4[1])
            p3 = p4
        steps1 += abs(p1[0] - p2[0]) + abs(p1[1] - p2[1])
        p1 = p2
    return best
...
if __name__ == "__main__":
    with open('input.txt', 'r') as input_file:
        ...
        print(f"Intersection : {ans[1]}, steps : {ans[0]}")
